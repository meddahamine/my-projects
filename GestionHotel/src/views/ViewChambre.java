...
public abstract class ViewChambre {
	...
	public static javax.swing.JTextField getViewForNumChamre(){
		return gui.utils.GUIFieldFactory.createSimpleTextField(10);
	}

	public static String getCaptionForNumChamre(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("", userParameters.getLang().getCodeLang());
		}
	...
	public static javax.swing.JTextField getViewForNumEtage(){
		return gui.utils.GUIFieldFactory.createSimpleTextField(10);
	}

	public static String getCaptionForNumEtage(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("", userParameters.getLang().getCodeLang());
		}
	...
	public static javax.swing.JComboBox getViewForCategorieChambre(){
		java.util.List<models.beans.CategorieChambre> list = models.daos.client.DAOCategorieChambre.getListInstances();
		javax.swing.JComboBox view = new gui.utils.GUIPanelFactory.JEditedComboBox(list.toArray());
		view.insertItemAt("Choisir CategorieChambre", 0);
		return view;
	}

	public static String getCaptionForCategorieChambre(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("", userParameters.getLang().getCodeLang());
		}
	...
	public static java.util.Vector<Object> getTableRow(models.beans.Chambre chambre, int numOrder){
		java.util.Vector<Object> row = new java.util.Vector<Object>();
		
		if (numOrder > 0)
			row.add(numOrder);
		
		row.add(chambre.getNumChamre());
		row.add(chambre.getNumEtage());
		row.add(chambre.getCategorieChambre());
	...
	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.Chambre> list, boolean withOrder){
		return getData(list, withOrder, 0);
	}

	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.Chambre> list, boolean withOrder, int numRowBase){
		java.util.Vector<java.util.Vector<Object>> data = new java.util.Vector<java.util.Vector<Object>>();
		if (list == null)
			return data;
		
		int numRow = numRowBase;
		for (models.beans.Chambre chambre : list){
			if (withOrder)
				numRow++;
			
			data.add(getTableRow(chambre , numRow));
		}
	...
	public static void exportToExcel(File file){
		exportToExcel(file, DAOChambre.getListInstances());
	}

	public static void exportToExcel(File file, java.util.List<Chambre> list){
		try{
			WritableWorkbook workbook = Workbook.createWorkbook(file);
			WritableSheet sheet = workbook.createSheet( "Liste des ViewChambres", 0);
			...
			for (Chambre chambre : list){
				int i = list.indexOf(chambre);
				label = new Label(0, ligneBas+i, String.valueOf(i), formatContentStr); sheet.addCell(label);
				label = new Label(1, ligneBas+i, chambre.getNumChamre().toString(), formatHeaderTableStr); sheet.addCell(label);
				label = new Label(2, ligneBas+i, chambre.getNumEtage().toString(), formatHeaderTableStr); sheet.addCell(label);
				label = new Label(3, ligneBas+i, chambre.getIdCategorieChambre().toString(), formatHeaderTableStr); sheet.addCell(label);
				label = new Label(4, ligneBas+i, chambre.getPrix().toString(), formatHeaderTableStr); sheet.addCell(label);
			}
		...
		try{
			...
			while (true){
				...
				try{
					...
					chambre.setNumChamre(value);
					value = sheet.getCell(2, ligne-1).getContents().trim();
					chambre.setNumEtage(value);
					value = sheet.getCell(3, ligne-1).getContents().trim();
					chambre.setIdCategorieChambre(Integer.parseInt(value));
					value = sheet.getCell(4, ligne-1).getContents().trim();
					chambre.setPrix(Double.parseDouble(value));
					queries.add(DAOChambre.getSQLWriting(chambre));
				}catch (Exception e){
					utils.StringUtils.printDebug(e);
				}
			}
	...
	public static void exportToPDF(File file){
		exportToPDF(file, DAOChambre.getListInstances());
	}

	public static void exportToPDF(File file, java.util.List<Chambre> list){
		try{
...
//			PdfContentByte canvas = writer.getDirectContent();
			...
			for (int numPage=0; numPage<nbPages; numPage++){
				...
				for (int i=0; i<maxItemsAtPage; i++){
					...
					if (index < list.size())
						chambre = list.get(index);
					
					cell = new PdfPCell(new Paragraph((chambre != null) ? chambre.getNumChamre().toString() : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);

					cell = new PdfPCell(new Paragraph((chambre != null) ? chambre.getNumEtage().toString() : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);

					cell = new PdfPCell(new Paragraph((chambre != null) ? (chambre.getCategorieChambre() == null ? "" : chambre.getCategorieChambre().toString()) : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);

					cell = new PdfPCell(new Paragraph((chambre != null) ? chambre.getPrix().toString() : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);
	...
	public static void exportToText(File file){
		exportToText(file, DAOChambre.getListInstances());
	}

	public static void exportToText(File file, java.util.List<Chambre> list){
		try{
	...
	public static void exportToWord(File file){
		exportToWord(file, DAOChambre.getListInstances());
	}

	public static void exportToWord(File file, java.util.List<Chambre> list){
		try{
	...
	public static javax.swing.JPanel getForm(javax.swing.JPanel panel){
		...
		javax.swing.JLabel labelNumChamre = new javax.swing.JLabel("");
		javax.swing.JLabel labelNumEtage = new javax.swing.JLabel("");
		javax.swing.JLabel labelCategorieChambre = new javax.swing.JLabel("");
		javax.swing.JLabel labelPrix = new javax.swing.JLabel("");
		
		javax.swing.JComponent componentNumChamre = views.ViewChambre.getViewForNumChamre();
		javax.swing.JComponent componentNumEtage = views.ViewChambre.getViewForNumEtage();
		javax.swing.JComponent componentCategorieChambre = views.ViewChambre.getViewForCategorieChambre();
		javax.swing.JComponent componentPrix = views.ViewChambre.getViewForPrix();
		...
		layout.setVerticalGroup(layout.createSequentialGroup()
			...
			.addGroup(layout.createParallelGroup()
				.addComponent(labelNumChamre)
				.addComponent(componentNumChamre, 20, 20, 20)
			)
			.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
			.addGroup(layout.createParallelGroup()
				.addComponent(labelNumEtage)
				.addComponent(componentNumEtage, 20, 20, 20)
			)
			.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
			.addGroup(layout.createParallelGroup()
				.addComponent(labelCategorieChambre)
				.addComponent(componentCategorieChambre, 20, 20, 20)
			)
			.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
			.addGroup(layout.createParallelGroup()
				.addComponent(labelPrix)
	...
	public static java.util.Map<String, gui.utils.GroupLayouter.LineForm> getFormSimple(javax.swing.JPanel panel){
		...
		lineForm = new gui.utils.GroupLayouter.LineForm(labelNumChamre, componentNumChamre);
		lineForm.setHeight(25);
		mapLabelsFields.put("numchamre", lineForm);
		lineForm = new gui.utils.GroupLayouter.LineForm(labelNumEtage, componentNumEtage);
		lineForm.setHeight(25);
		mapLabelsFields.put("numetage", lineForm);
		lineForm = new gui.utils.GroupLayouter.LineForm(labelCategorieChambre, componentCategorieChambre);
		lineForm.setHeight(25);
		mapLabelsFields.put("categoriechambre", lineForm);
		...
		javax.swing.JComponent[][] matrice ={
			{labelNumChamre, componentNumChamre},
			{labelNumEtage, componentNumEtage},
			{labelCategorieChambre, componentCategorieChambre},
	...
	public static class DlgAddModifyChambre extends javax.swing.JDialog{
		...
		protected models.beans.Chambre chambre = null;
		
		public DlgAddModifyChambre(java.awt.Window parent, String title){
			this(parent, title, null);
		}
		
		public DlgAddModifyChambre(java.awt.Window parent, String title, models.beans.Chambre chambre){
			super(parent);
			initComponents();
		...
		protected void bValiderActionPerformed(){
			...
			chambre.setNumChamre(mapAttributLineForm.get("numchamre").getValueAsString());
			chambre.setNumEtage(mapAttributLineForm.get("numetage").getValueAsString());
			
			int idCategorieChambre = chambre.getIdCategorieChambre().intValue();
			if (mapAttributLineForm.get("categoriechambre").getValue() != null && mapAttributLineForm.get("categoriechambre").getValue() instanceof models.beans.CategorieChambre){
				idCategorieChambre = ((models.beans.CategorieChambre)(mapAttributLineForm.get("categoriechambre").getValue())).getId().intValue();
			}
			chambre.setIdCategorieChambre(idCategorieChambre);
			chambre.setPrix(mapAttributLineForm.get("prix").getValueAsDouble());
			
			chambre.setId((Integer)communication.SocketCommunicator.getInstance().sendQuery(chambre));
			this.setChambre(chambre);
		...
		protected void bCancelActionPerformed(){
			this.setChambre(null);
			this.setVisible(false);
		}
		
		public void setChambre(models.beans.Chambre chambre) {
			this.chambre = chambre;
			updateForm();
		}
		
		public models.beans.Chambre getChambre() {
			return this.chambre;
		}
		
		protected void emptyForm(){
			mapAttributLineForm.get("numchamre").setValue(null);
			mapAttributLineForm.get("numetage").setValue(null);
			mapAttributLineForm.get("categoriechambre").setValue(null);
		...
		protected void updateForm(){
			...
			mapAttributLineForm.get("numchamre").setValue(chambre.getNumChamre());
			mapAttributLineForm.get("numetage").setValue(chambre.getNumEtage());
			mapAttributLineForm.get("categoriechambre").setValue(chambre.getCategorieChambre());
			mapAttributLineForm.get("prix").setValue(chambre.getPrix());
		}
	}
