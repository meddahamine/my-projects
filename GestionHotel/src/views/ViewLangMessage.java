...
public abstract class ViewLangMessage {
	...
	public static javax.swing.JTextField getViewForMessage(){
		return gui.utils.GUIFieldFactory.createSimpleTextField(1000);
	}

	public static String getCaptionForMessage(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("Message", userParameters.getLang().getCodeLang());
		}
	...
	public static java.util.Vector<Object> getTableRow(models.beans.LangMessage langMessage, int numOrder){
		java.util.Vector<Object> row = new java.util.Vector<Object>();
		
		if (numOrder > 0)
			row.add(numOrder);
		
		row.add(langMessage.getMessage());
	...
	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.LangMessage> list, boolean withOrder){
		return getData(list, withOrder, 0);
	}

	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.LangMessage> list, boolean withOrder, int numRowBase){
		java.util.Vector<java.util.Vector<Object>> data = new java.util.Vector<java.util.Vector<Object>>();
		if (list == null)
			return data;
		
		int numRow = numRowBase;
		for (models.beans.LangMessage langMessage : list){
			if (withOrder)
				numRow++;
			
			data.add(getTableRow(langMessage , numRow));
		}
	...
	public static void exportToExcel(File file){
		exportToExcel(file, DAOLangMessage.getListInstances());
	}

	public static void exportToExcel(File file, java.util.List<LangMessage> list){
		try{
			WritableWorkbook workbook = Workbook.createWorkbook(file);
			WritableSheet sheet = workbook.createSheet( "Liste des ViewLangMessages", 0);
			...
			label = new Label(1, 0, "Message", formatHeaderTableStr); sheet.addCell(label);
			
			int ligneBas = 1;
			for (LangMessage langMessage : list){
				int i = list.indexOf(langMessage);
				label = new Label(0, ligneBas+i, String.valueOf(i), formatContentStr); sheet.addCell(label);
				label = new Label(1, ligneBas+i, langMessage.getMessage().toString(), formatHeaderTableStr); sheet.addCell(label);
			}
		...
		try{
			...
			while (true){
				...
				try{
					...
					langMessage.setMessage(value);
					queries.add(DAOLangMessage.getSQLWriting(langMessage));
				}catch (Exception e){
					utils.StringUtils.printDebug(e);
				}
			}
	...
	public static void exportToPDF(File file){
		exportToPDF(file, DAOLangMessage.getListInstances());
	}

	public static void exportToPDF(File file, java.util.List<LangMessage> list){
		try{
...
//			PdfContentByte canvas = writer.getDirectContent();
			...
			for (int numPage=0; numPage<nbPages; numPage++){
				docPDF.newPage();
				Paragraph header = new Paragraph("Messages En Français", fontHVCA15_IB);
				...
				cell = new PdfPCell(new Paragraph("Message", fontTR10_B_WHITE));
				...
				for (int i=0; i<maxItemsAtPage; i++){
					...
					if (index < list.size())
						langMessage = list.get(index);
					
					cell = new PdfPCell(new Paragraph((langMessage != null) ? langMessage.getMessage().toString() : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);
	...
	public static void exportToText(File file){
		exportToText(file, DAOLangMessage.getListInstances());
	}

	public static void exportToText(File file, java.util.List<LangMessage> list){
		try{
	...
	public static void exportToWord(File file){
		exportToWord(file, DAOLangMessage.getListInstances());
	}

	public static void exportToWord(File file, java.util.List<LangMessage> list){
		try{
	...
	public static javax.swing.JPanel getForm(javax.swing.JPanel panel){
		...
		javax.swing.JLabel labelMessage = new javax.swing.JLabel("Message");
		
		javax.swing.JComponent componentMessage = views.ViewLangMessage.getViewForMessage();
	...
	public static class DlgAddModifyLangMessage extends javax.swing.JDialog{
		...
		protected models.beans.LangMessage langMessage = null;
		
		public DlgAddModifyLangMessage(java.awt.Window parent, String title){
			this(parent, title, null);
		}
		
		public DlgAddModifyLangMessage(java.awt.Window parent, String title, models.beans.LangMessage langMessage){
			super(parent);
			initComponents();
			this.setLangMessage(langMessage);
		}
		...
		protected void bValiderActionPerformed(){
			LangMessage langMessage = this.getLangMessage();
			if (langMessage == null){
				langMessage = new LangMessage();
			}
			
			langMessage.setMessage(mapAttributLineForm.get("message").getValueAsString());
			
			langMessage.setId((Integer)communication.SocketCommunicator.getInstance().sendQuery(langMessage));
			this.setLangMessage(langMessage);
		...
		protected void bCancelActionPerformed(){
			this.setLangMessage(null);
			this.setVisible(false);
		}
		
		public void setLangMessage(models.beans.LangMessage langMessage) {
			this.langMessage = langMessage;
			updateForm();
		}
		
		public models.beans.LangMessage getLangMessage() {
		...
		protected void updateForm(){
			...
			mapAttributLineForm.get("message").setValue(langMessage.getMessage());
		}
	}
