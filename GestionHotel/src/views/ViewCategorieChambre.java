...
public abstract class ViewCategorieChambre {
	...
	public static javax.swing.JTextField getViewForLibelle(){
		return gui.utils.GUIFieldFactory.createSimpleTextField(55);
	}

	public static String getCaptionForLibelle(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("", userParameters.getLang().getCodeLang());
		}
	...
	public static java.util.Vector<Object> getTableRow(models.beans.CategorieChambre categorieChambre, int numOrder){
		java.util.Vector<Object> row = new java.util.Vector<Object>();
		
		if (numOrder > 0)
			row.add(numOrder);
		
		row.add(categorieChambre.getLibelle());
		
		row.add(categorieChambre);
		
		return row;
	}

	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.CategorieChambre> list, boolean withOrder){
		return getData(list, withOrder, 0);
	}

	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.CategorieChambre> list, boolean withOrder, int numRowBase){
		java.util.Vector<java.util.Vector<Object>> data = new java.util.Vector<java.util.Vector<Object>>();
		if (list == null)
			return data;
		
		int numRow = numRowBase;
		for (models.beans.CategorieChambre categorieChambre : list){
			if (withOrder)
				numRow++;
			
			data.add(getTableRow(categorieChambre , numRow));
		}
	...
	public static void exportToExcel(File file){
		exportToExcel(file, DAOCategorieChambre.getListInstances());
	}

	public static void exportToExcel(File file, java.util.List<CategorieChambre> list){
		try{
			WritableWorkbook workbook = Workbook.createWorkbook(file);
			WritableSheet sheet = workbook.createSheet( "Liste des ViewCategorieChambres", 0);
			...
			for (CategorieChambre categorieChambre : list){
				int i = list.indexOf(categorieChambre);
				label = new Label(0, ligneBas+i, String.valueOf(i), formatContentStr); sheet.addCell(label);
				label = new Label(1, ligneBas+i, categorieChambre.getLibelle().toString(), formatHeaderTableStr); sheet.addCell(label);
			}
		...
		try{
			...
			while (true){
				...
				try{
					...
					categorieChambre.setLibelle(value);
					queries.add(DAOCategorieChambre.getSQLWriting(categorieChambre));
				}catch (Exception e){
					utils.StringUtils.printDebug(e);
				}
			}
	...
	public static void exportToPDF(File file){
		exportToPDF(file, DAOCategorieChambre.getListInstances());
	}

	public static void exportToPDF(File file, java.util.List<CategorieChambre> list){
		try{
...
//			PdfContentByte canvas = writer.getDirectContent();
			...
			for (int numPage=0; numPage<nbPages; numPage++){
				...
				for (int i=0; i<maxItemsAtPage; i++){
					...
					CategorieChambre categorieChambre = null;
					if (index < list.size())
						categorieChambre = list.get(index);
					
					cell = new PdfPCell(new Paragraph((categorieChambre != null) ? categorieChambre.getLibelle().toString() : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);
	...
	public static void exportToText(File file){
		exportToText(file, DAOCategorieChambre.getListInstances());
	}

	public static void exportToText(File file, java.util.List<CategorieChambre> list){
		try{
	...
	public static void exportToWord(File file){
		exportToWord(file, DAOCategorieChambre.getListInstances());
	}

	public static void exportToWord(File file, java.util.List<CategorieChambre> list){
		try{
	...
	public static javax.swing.JPanel getForm(javax.swing.JPanel panel){
		...
		javax.swing.JLabel labelLibelle = new javax.swing.JLabel("");
		
		javax.swing.JComponent componentLibelle = views.ViewCategorieChambre.getViewForLibelle();
		...
		layout.setVerticalGroup(layout.createSequentialGroup()
			...
			.addGroup(layout.createParallelGroup()
				.addComponent(labelLibelle)
				.addComponent(componentLibelle, 20, 20, 20)
			)
			.addGap(1, 1, Short.MAX_VALUE)
		);
	...
	public static java.util.Map<String, gui.utils.GroupLayouter.LineForm> getFormSimple(javax.swing.JPanel panel){
		...
		lineForm = new gui.utils.GroupLayouter.LineForm(labelLibelle, componentLibelle);
		lineForm.setHeight(25);
		mapLabelsFields.put("libelle", lineForm);

		javax.swing.JComponent[][] matrice ={
			{labelLibelle, componentLibelle},
		};
	...
	public static class DlgAddModifyCategorieChambre extends javax.swing.JDialog{
		...
		protected models.beans.CategorieChambre categorieChambre = null;
		
		public DlgAddModifyCategorieChambre(java.awt.Window parent, String title){
			this(parent, title, null);
		}
		
		public DlgAddModifyCategorieChambre(java.awt.Window parent, String title, models.beans.CategorieChambre categorieChambre){
			super(parent);
			initComponents();
		...
		protected void bValiderActionPerformed(){
			CategorieChambre categorieChambre = this.getCategorieChambre();
			if (categorieChambre == null){
				categorieChambre = new CategorieChambre();
			}
			
			categorieChambre.setLibelle(mapAttributLineForm.get("libelle").getValueAsString());
			
			categorieChambre.setId((Integer)communication.SocketCommunicator.getInstance().sendQuery(categorieChambre));
			this.setCategorieChambre(categorieChambre);
		...
		protected void bCancelActionPerformed(){
			this.setCategorieChambre(null);
			this.setVisible(false);
		}
		
		public void setCategorieChambre(models.beans.CategorieChambre categorieChambre) {
			this.categorieChambre = categorieChambre;
			updateForm();
		}
		
		public models.beans.CategorieChambre getCategorieChambre() {
			return this.categorieChambre;
		}
		
		protected void emptyForm(){
			mapAttributLineForm.get("libelle").setValue(null);
		}
		...
		protected void updateForm(){
			...
			mapAttributLineForm.get("libelle").setValue(categorieChambre.getLibelle());
		}
	}
