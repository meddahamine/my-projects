...
public abstract class ViewEventsLog {
	...
	public static javax.swing.JScrollPane getViewForEvenement(){
		return gui.utils.GUIFieldFactory.createTextAreaWithScroll();
	}

	public static String getCaptionForEvenement(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("", userParameters.getLang().getCodeLang());
		}
	...
	public static javax.swing.JComboBox getViewForConnexionsLog(){
		java.util.List<models.beans.ConnexionsLog> list = models.daos.client.DAOConnexionsLog.getListInstances();
		javax.swing.JComboBox view = new gui.utils.GUIPanelFactory.JEditedComboBox(list.toArray());
		view.insertItemAt("Choisir ConnexionsLog", 0);
		return view;
	}

	public static String getCaptionForConnexionsLog(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("", userParameters.getLang().getCodeLang());
		}
	...
	public static gui.utils.GUIDate getViewForDate(){
		return new gui.utils.GUIDate();
	}

	public static String getCaptionForDate(){
		if (userParameters != null && userParameters.getLang() != null){
			return models.daos.client.DAOTranslation.translate("", userParameters.getLang().getCodeLang());
		}
	...
	public static java.util.Vector<Object> getTableRow(models.beans.EventsLog eventsLog, int numOrder){
		java.util.Vector<Object> row = new java.util.Vector<Object>();
		
		if (numOrder > 0)
			row.add(numOrder);
		
		row.add(eventsLog.getEvenement());
		row.add(eventsLog.getConnexionsLog());
		row.add(eventsLog.getDate());
	...
	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.EventsLog> list, boolean withOrder){
		return getData(list, withOrder, 0);
	}

	public static java.util.Vector<java.util.Vector<Object>> getData(java.util.List<models.beans.EventsLog> list, boolean withOrder, int numRowBase){
		java.util.Vector<java.util.Vector<Object>> data = new java.util.Vector<java.util.Vector<Object>>();
		if (list == null)
			return data;
		
		int numRow = numRowBase;
		for (models.beans.EventsLog eventsLog : list){
			if (withOrder)
				numRow++;
			
			data.add(getTableRow(eventsLog , numRow));
		}
	...
	public static void exportToExcel(File file){
		exportToExcel(file, DAOEventsLog.getListInstances());
	}

	public static void exportToExcel(File file, java.util.List<EventsLog> list){
		try{
			WritableWorkbook workbook = Workbook.createWorkbook(file);
			WritableSheet sheet = workbook.createSheet( "Liste des ViewEventsLogs", 0);
			...
			int ligneBas = 1;
			for (EventsLog eventsLog : list){
				int i = list.indexOf(eventsLog);
				label = new Label(0, ligneBas+i, String.valueOf(i), formatContentStr); sheet.addCell(label);
				label = new Label(1, ligneBas+i, eventsLog.getEvenement().toString(), formatHeaderTableStr); sheet.addCell(label);
				label = new Label(2, ligneBas+i, eventsLog.getIdConnexionsLog().toString(), formatHeaderTableStr); sheet.addCell(label);
				label = new Label(3, ligneBas+i, eventsLog.getDate().toString(), formatHeaderTableStr); sheet.addCell(label);
			}
		...
		try{
			...
			while (true){
				...
				try{
					...
					eventsLog.setEvenement(value);
					value = sheet.getCell(2, ligne-1).getContents().trim();
					eventsLog.setIdConnexionsLog(Integer.parseInt(value));
					value = sheet.getCell(3, ligne-1).getContents().trim();
					eventsLog.setDate(utils.StringUtils.getTimestampFromString(value));
					queries.add(DAOEventsLog.getSQLWriting(eventsLog));
				}catch (Exception e){
					utils.StringUtils.printDebug(e);
				}
			}
	...
	public static void exportToPDF(File file){
		exportToPDF(file, DAOEventsLog.getListInstances());
	}

	public static void exportToPDF(File file, java.util.List<EventsLog> list){
		try{
...
//			PdfContentByte canvas = writer.getDirectContent();
			...
			for (int numPage=0; numPage<nbPages; numPage++){
				docPDF.newPage();
				Paragraph header = new Paragraph("Journal des Évènements", fontHVCA15_IB);
				...
				for (int i=0; i<maxItemsAtPage; i++){
					...
					if (index < list.size())
						eventsLog = list.get(index);
					
					cell = new PdfPCell(new Paragraph((eventsLog != null) ? eventsLog.getEvenement().toString() : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);

					cell = new PdfPCell(new Paragraph((eventsLog != null) ? (eventsLog.getConnexionsLog() == null ? "" : eventsLog.getConnexionsLog().toString()) : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);

					cell = new PdfPCell(new Paragraph((eventsLog != null) ? eventsLog.getDate().toString() : " ", fontTR10));
					cell.setBorderColor(BaseColor.WHITE);
					pdfTable.addCell(cell);
	...
	public static void exportToText(File file){
		exportToText(file, DAOEventsLog.getListInstances());
	}

	public static void exportToText(File file, java.util.List<EventsLog> list){
		try{
	...
	public static void exportToWord(File file){
		exportToWord(file, DAOEventsLog.getListInstances());
	}

	public static void exportToWord(File file, java.util.List<EventsLog> list){
		try{
	...
	public static javax.swing.JPanel getForm(javax.swing.JPanel panel){
		...
		javax.swing.JLabel labelEvenement = new javax.swing.JLabel("");
		javax.swing.JLabel labelConnexionsLog = new javax.swing.JLabel("");
		javax.swing.JLabel labelDate = new javax.swing.JLabel("");
		
		javax.swing.JComponent componentEvenement = views.ViewEventsLog.getViewForEvenement();
		javax.swing.JComponent componentConnexionsLog = views.ViewEventsLog.getViewForConnexionsLog();
		javax.swing.JComponent componentDate = views.ViewEventsLog.getViewForDate();
		...
		layout.setVerticalGroup(layout.createSequentialGroup()
			...
			.addGroup(layout.createParallelGroup()
				.addComponent(labelEvenement)
				.addComponent(componentEvenement, 20, 20, 20)
			)
			.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
			.addGroup(layout.createParallelGroup()
				.addComponent(labelConnexionsLog)
				.addComponent(componentConnexionsLog, 20, 20, 20)
			)
			.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
			.addGroup(layout.createParallelGroup()
				.addComponent(labelDate)
				.addComponent(componentDate, 20, 20, 20)
			)
			.addGap(1, 1, Short.MAX_VALUE)
		);
	...
	public static java.util.Map<String, gui.utils.GroupLayouter.LineForm> getFormSimple(javax.swing.JPanel panel){
		...
		lineForm = new gui.utils.GroupLayouter.LineForm(labelEvenement, componentEvenement);
		lineForm.setHeight(25);
		mapLabelsFields.put("evenement", lineForm);
		lineForm = new gui.utils.GroupLayouter.LineForm(labelConnexionsLog, componentConnexionsLog);
		lineForm.setHeight(25);
		mapLabelsFields.put("connexionslog", lineForm);
		lineForm = new gui.utils.GroupLayouter.LineForm(labelDate, componentDate);
		lineForm.setHeight(25);
		mapLabelsFields.put("date", lineForm);

		javax.swing.JComponent[][] matrice ={
			{labelEvenement, componentEvenement},
			{labelConnexionsLog, componentConnexionsLog},
			{labelDate, componentDate},
		};
	...
	public static class DlgAddModifyEventsLog extends javax.swing.JDialog{
		...
		protected models.beans.EventsLog eventsLog = null;
		
		public DlgAddModifyEventsLog(java.awt.Window parent, String title){
			this(parent, title, null);
		}
		
		public DlgAddModifyEventsLog(java.awt.Window parent, String title, models.beans.EventsLog eventsLog){
			super(parent);
			initComponents();
			this.setEventsLog(eventsLog);
		}
		...
		protected void bValiderActionPerformed(){
			EventsLog eventsLog = this.getEventsLog();
			if (eventsLog == null){
				eventsLog = new EventsLog();
			}
			
			eventsLog.setEvenement(mapAttributLineForm.get("evenement").getValueAsString());
			
			int idConnexionsLog = eventsLog.getIdConnexionsLog().intValue();
			if (mapAttributLineForm.get("connexionslog").getValue() != null && mapAttributLineForm.get("connexionslog").getValue() instanceof models.beans.ConnexionsLog){
				idConnexionsLog = ((models.beans.ConnexionsLog)(mapAttributLineForm.get("connexionslog").getValue())).getId().intValue();
			}
			eventsLog.setIdConnexionsLog(idConnexionsLog);
			eventsLog.setDate(mapAttributLineForm.get("date").getValueAsTimestamp());
			
			eventsLog.setId((Integer)communication.SocketCommunicator.getInstance().sendQuery(eventsLog));
			this.setEventsLog(eventsLog);
		...
		protected void bCancelActionPerformed(){
			this.setEventsLog(null);
			this.setVisible(false);
		}
		
		public void setEventsLog(models.beans.EventsLog eventsLog) {
			this.eventsLog = eventsLog;
			updateForm();
		}
		
		public models.beans.EventsLog getEventsLog() {
			return this.eventsLog;
		}
		
		protected void emptyForm(){
			mapAttributLineForm.get("evenement").setValue(null);
			mapAttributLineForm.get("connexionslog").setValue(null);
			mapAttributLineForm.get("date").setValue(null);
		}
		...
		protected void updateForm(){
			...
			mapAttributLineForm.get("evenement").setValue(eventsLog.getEvenement());
			mapAttributLineForm.get("connexionslog").setValue(eventsLog.getConnexionsLog());
			mapAttributLineForm.get("date").setValue(eventsLog.getDate());
		}
	}
